...
class Map implements Comparable<Map> {
    ...
    public Map(int row, int col, int broken) {
        this.row = row;
        this.col = col;
        this.broken = broken;
    }
    ...
    @Override
    public int compareTo(Map temp) { return this.broken - temp.getBroken(); }
}
                ...
                if(currentRow >= 0 && currentCol >= 0 && currentRow < row && currentCol < col) {
                    if(!visited[currentRow][currentCol]) {
                        if(map[currentRow][currentCol] == 1) {
                            ...
                            queue.offer(new Map(currentRow, currentCol, current.getBroken() + 1));
                        } else {
                            visited[currentRow][currentCol] = true;
                            queue.offer(new Map(currentRow, currentCol, current.getBroken()));
                        }
                    }
                }
            }
